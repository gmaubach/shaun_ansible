...
#    v.gui = true
  ...
  config.vm.define :controller, primary: true do |controller|
    ...
    controller.vm.network "private_network", ip: "192.168.63.100"

    # Public Local Network
    controller.vm.network "public_network", bridge: "enp6s0"
  ...
  (1..2).each do |i|
    config.vm.define "web-00#{i}" do |node|
      ...
      node.vm.network "private_network", ip: "192.168.63.11#{i}"
    end
  end
  ...
  (1..1).each do |i|
    config.vm.define "lb-00#{i}" do |node|
      ...
      node.vm.network "private_network", ip: "192.168.63.12#{i}"
    end
  end
  ...
  (1..1).each do |i|
    config.vm.define "db-00#{i}" do |node|
      ...
      node.vm.network "private_network", ip: "192.168.63.13#{i}"
    end
  end
